...
class ConfigurationHelper {
  ...
  public function createConfiguration(array $post) {
    ...
    if ($this->checkDuplicateItems($args) == TRUE) {
      $return = ['status' => 'error', 'message' => t('Machine name already exists in this combination')];
    }
    else {
      /* Must have value $name, $machine_name and $config_value. */
      if ((!empty($name)) && (!empty($machine_name)) && (!empty($config_value))) {
        try {
          $id = $this->database->insert('custom_configuration')
                  ->fields([
          ...
          $message = t('Configuration saved successfully. Machine name @machine_name.', ['@machine_name' => $machine_name]);
        ...
        $return = ['status' => 'error', 'message' => t('Required field can not empty')];
      }
    }
    return $return;
  }
  ...
  public function getMessageByCode($code) {
    if ($code == '23000') {
      ...
      $return = ['status' => 'error', 'message' => t('Configuration cannot be saved')];
    }
    return $return;
  }
    ...
    if ($this->checkDuplicateItems($args) == TRUE) {
      ...
      if ($name !== NULL && $config_value !== NULL && $status !== NULL) {
        try {
          ...
          if ($row_updated > 0) {
            $return = ['status' => 'status', 'message' => t('Configuration saved successfully.')];
          }
          else {
            $return = ['status' => 'error', 'message' => t('Configuration cannot be saved.')];
          }
  ...
  public function deleteValue($config_id) {
    ...
    if ($del > 0) {
      return ['status' => 'status', 'message' => t('Configuration deleted successfully.')];
    }
    else {
      return ['status' => 'error', 'message' => t('Configuration cannot be deleted.')];
    }
  }
  ...
  public function getDomains() {
    ...
    if ($moduleHandler->moduleExists('domain')) {
      ...
      $this->activeDomain = $negotiator->getActiveDomain()->id();
